...
BEGIN
    ...
    FROM Payments p
    INNER JOIN Rides r ON p.RideID = r.RideID
...
DELIMITER $$
CREATE PROCEDURE calc_total_sedan_price(OUT total_price DECIMAL(10,2))
BEGIN
    SELECT SUM(Fare) INTO total_price
    FROM Rides
    WHERE VehicleID IN (SELECT VehicleID FROM Vehicles WHERE VehicleType = 'Sedan');
END$$
DELIMITER ;

CALL calc_total_sedan_price(@total);
SELECT @total AS total_sedan_price;


DELIMITER $$
CREATE PROCEDURE print_user_names()
BEGIN
    DECLARE user_name VARCHAR(255);
    DECLARE done INT DEFAULT FALSE;
    DECLARE user_cursor CURSOR FOR SELECT UName FROM Users;
