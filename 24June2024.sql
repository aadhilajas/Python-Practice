...
-- Start a transaction to insert a new ride and payment record
START TRANSACTION;

INSERT INTO Rides (UserID, DriverID, VehicleID, PickupLocation, DropoffLocation, PickupTime, DropoffTime, Fare, Status)
VALUES (1, 1, 1, 'Kochi Airport', 'Marine Drive', '2024-06-24 15:00:00', '2024-06-24 15:30:00', 300.00, 'Completed');

INSERT INTO Payments (RideID, UserID, Amount, PaymentMethod, PaymentStatus, TransactionID)
VALUES (6, 1, 300.00, 'Credit Card', 'Paid', 'TXN234567890');

-- Commit the transaction to save changes permanently
COMMIT;

-- Trigger to deactivate driver when vehicle is changed
DELIMITER //
CREATE TRIGGER deactivate_driver_after_update
AFTER UPDATE ON Drivers
FOR EACH ROW
BEGIN
    IF NEW.VehicleID <> OLD.VehicleID THEN
        UPDATE Drivers
        SET IsActive = FALSE
...
-- Trigger to update ride fare automatically based on changes to vehicle capacity
DELIMITER //
CREATE TRIGGER update_ride_fare_after_update
AFTER UPDATE ON Vehicles
FOR EACH ROW
BEGIN
    DECLARE new_capacity INT;
    DECLARE fare_multiplier DECIMAL(10, 2);

    SET new_capacity = NEW.Capacity;

    IF new_capacity > 5 THEN
        SET fare_multiplier = 1.2; 
    ELSE
        SET fare_multiplier = 1.0;
    END IF;

    UPDATE Rides
    SET Fare = Fare * fare_multiplier
    WHERE VehicleID = NEW.VehicleID;
END //
DELIMITER ;

-- View to display completed rides along with user feedback
CREATE OR REPLACE VIEW CompletedRidesWithFeedback AS
SELECT R.RideID, R.UserID, U.UName AS UserName, R.DriverID, D.DName AS DriverName, R.VehicleID, V.VehicleType,
       R.PickupLocation, R.DropoffLocation, R.PickupTime, R.DropoffTime, R.Fare, F.Rating, F.Comments
FROM Rides R
JOIN Users U ON R.UserID = U.UserID
JOIN Drivers D ON R.DriverID = D.DriverID
JOIN Vehicles V ON R.VehicleID = V.VehicleID
LEFT JOIN Feedback F ON R.RideID = F.RideID
WHERE R.Status = 'Completed';

-- View to display active drivers along with their vehicle details
CREATE OR REPLACE VIEW ActiveDrivers AS
SELECT D.DriverID, D.DName, D.Email, D.Address, D.PhoneNumber, V.VehicleID, V.VehicleType, V.RegistrationNumber
FROM Drivers D
